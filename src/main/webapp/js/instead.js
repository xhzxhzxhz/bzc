...
//文本域显示字数
function textNumber(){
	$('#import').text($('#instead_content').val().length);	
}
...
function invalid_clean() {
	...
	$("#instead_content").val('');
	$("#instead_replyno").val('');
	$("#instead_replyname").val('');
}

function invalid_submit() {
	var standno = $("#standardNumber").val();
	var standname = $("#validationName").val();
	var content = $("#instead_content").val();
	var replyno = $("#instead_replyno").val();
	var replyname = $("#instead_replyname").val();
	var id = $("#standardNumber").attr("name");
	
	var mustfill = isempty(standno,"standardNumber");
	var mustfill1 = isempty(standname,"validationName");
	var mustfill2 = isempty(replyno,"instead_replyno");
	var mustfill3 = isempty(replyname,"instead_replyname");
	if(mustfill && mustfill1 && mustfill2 && mustfill3){
		var data = {
				"standardId" : id,
				...
				"comment" : content,
				"suggestStatus" : 2,
				"replaystandName":replyname,
				"replaystandNo" :replyno,
				"status" : 1
		}
	;
